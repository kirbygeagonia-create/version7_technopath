...
def make_data_dict_table(table_name, info):
    ...
    r2.cells[1].text = f"Displays the {info['model_name']} information."
    ...
    for field in info['fields']:
        ...
        tr.cells[3].text = "No" if field['null'] else "Yes" # "Null" usually means "Is it allowed to be null?" No -> not null. Wait. "Yes" means it can be null.
...
# Strategy 1: Delete all text paragraphs starting with "Table <number>:" inside the whole document
tables_regex = re.compile(r'^Table \d+: .*')

paragraphs_to_remove = []
for p in doc.paragraphs:
    if tables_regex.match(p.text.strip()):
...
# Strategy 2: Collect tables to replace
table_actions = []
use_case_data = []
    ...
    if typ == "USE_CASE":
        ...
        if not data_dict_inserted:
            ...
            for t_name, t_info in schema.items():
                if t_name:
                    # Injects Explicit ATTENDX Table Captions
                    p_cap = scratch.add_paragraph()
                    p_cap.add_run(f"Table 3.{table_counter}: Data Dictionary of {t_info['model_name']} Table of the Technopath System").bold = True
                    new_elements.append(p_cap._element)
                    
                    new_elements.append(make_data_dict_table(t_name, t_info))
                    
                    p_space = scratch.add_paragraph()
                    new_elements.append(p_space._element)
                    table_counter += 1
...
doc.save('Technopath_Document_Final_V3.docx')
print("Successfully generated revised Technopath_Document_Final_V3.docx!")
